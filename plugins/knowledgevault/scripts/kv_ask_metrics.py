...
# Strict pattern: heading followed (within ~800 chars) by units or EPS cue
STRICT_HEADING = re.compile(
...
# Row name patterns (allow table prefix chars)
PFX = r"^[^\w]{0,6}"
ROW_ALIASES = {
...
DEFAULT_K = 30
DEFAULT_TOP = 8
WINDOW_BEFORE = 300      # small look-back to avoid MD&A preface
WINDOW_AFTER  = 50000
...
def safe_read(path: Path, max_bytes: int = 8_000_000) -> str:
    try: return path.read_text("utf-8", errors="ignore")[:max_bytes]
    except: return ""
...
def likely_table(text: str) -> bool:
    t = text.lower()
    return ("(in millions" in t or "in millions" in t or
            "(in thousands" in t or "in thousands" in t or
            "except per share" in t or text.count("  ")>10 or "|" in text)

def unit_scale(text: str) -> int:
    t = text.lower()
    if "in millions" in t: return 1_000_000
    if "in thousands" in t: return 1_000
    return 1
...
def _find_strict_income_windows(raw: str) -> List[Dict[str,int]]:
    """Find headings where units/EPS cues appear within ~800 chars forward."""
    wins=[]
    low = raw.lower()
...
def is_percent_table(snippet: str) -> bool:
    """Detect MD&A percent-of-revenue tables aggressively."""
    t = snippet.lower()
    # Strong phrase
    if "expressed as a percentage of revenue" in t:
        # only treat as percent table if no units/EPS cues within the forward slice
        if has_units(t) or has_eps(t): return False
        return True
    # If tons of % and no units/EPS and very few $ within the first 2k chars → percent table
    head = snippet[:2000]
    perc = head.count("%")
    dollars = head.count("$")
...
def _has_key_rows(s: str) -> bool:
    t=s.lower()
    has_rev = re.search(r"\b(net\s+)?revenue\b|\bsales\b|\btotal\s+revenue\b", t) is not None
    has_net = re.search(r"\bnet\s+(income|loss|earnings)\b", t) is not None
    return has_rev and has_net

def build_income_context_from_files(files: List[Path], aliases: List[str], primary: str, debug: bool):
    contexts, citations = [], []
    for fp in files:
        raw = safe_read(fp)
        if not raw or not file_is_company(fp, raw, aliases, primary): continue

        wins = _find_strict_income_windows(raw)
        if not wins:
            wins = _find_generic_income_windows(raw)
        ...
        for w in wins:
            ...
            fwd = raw[anchor:end]        # gate on forward slice only
            snippet = raw[start:end]     # keep full for context

            # Force-keep for true statements
            force_keep = has_units(fwd) and _has_key_rows(fwd) and likely_table(fwd)

            if not force_keep and is_percent_table(fwd):
                if debug:
                    print(f"[DEBUG] anchor@{anchor} heading='{w['heading']}' decision=DROP reason=percent-table", file=sys.stderr)
                continue

            if not _has_key_rows(fwd):
                if debug: print(f"[DEBUG] anchor@{anchor} heading='{w['heading']}' decision=DROP reason=no-rev/net-income", file=sys.stderr)
                continue
            if not likely_table(fwd):
                if debug: print(f"[DEBUG] anchor@{anchor} heading='{w['heading']}' decision=DROP reason=not-tableish", file=sys.stderr)
                continue

            if debug:
                peek = re.sub(r"\s+"," ", fwd[:480])[:240]
                print(f"[DEBUG] anchor@{anchor} heading='{w['heading']}' decision=KEEP reason={'force-keep' if force_keep else 'ok'} | peek: '{peek}...'", file=sys.stderr)
...
def _find_numeric_trail(parts: List[str]) -> List[str]:
    ...
    for p in reversed(parts):
        if re.search(r"[0-9]", p): nums.append(p)
        elif nums: break
...
def parse_income_table_block(block: str, latest_on: str = "left") -> Dict[str, Any]:
    ...
    for ln in lines:
        ...
        for k,regs in comp.items():
            if any(r.match(low) for r in regs): row=k; break
        if not row: continue
        ...
        if val is not None: metrics[row]=val
    return metrics
...
def chunk_text(text: str, maxlen=1500) -> List[str]:
    ...
    for block in re.split(r"\n\s*\n", text):
        b=block.strip()
        if not b: continue
        if len(b)<=maxlen: parts.append(b)
        else:
            for i in range(0,len(b),maxlen): parts.append(b[i:i+maxlen])
    return parts

def score_chunk(text: str, aliases: List[str]) -> int:
    t=text.lower()
    tags=["consolidated statements of income","consolidated statement of income",
          "statements of operations","statement of operations","income statement",
          "net revenue","revenue","sales","cost of revenue","cost of goods sold",
          "operating income","operating loss","net income","net loss",
          "earnings per share","basic","diluted","(in millions","in millions","in thousands"]
    score=sum(tag in t for tag in tags)
    if any(a.lower() in t for a in aliases): score+=1
    if likely_table(text): score+=2
    if ("consolidated statements of income" in t or
        "statements of operations" in t or "income statement" in t): score+=3
    return score

def prefer_10k(filename: str) -> int:
    fn=filename.lower()
    if "_10-k" in fn or re.search(r"\b10[-\s]?k\b", fn): return 3
    if "_10-q" in fn or re.search(r"\b10[-\s]?q\b", fn): return 1
    return 0
...
def pick_top_chunks(index_dir: Path, company: str, k: int, top: int, debug: bool) -> List[Dict[str,Any]]:
    ...
    if debug: print(f"[DEBUG] Scanning {len(files)} files under {index_dir}", file=sys.stderr)
    ...
    for fp in files:
        ...
        for ch in chunk_text(raw, maxlen=1500):
            if not any(a.lower() in ch.lower() for a in aliases): continue
            s = score_chunk(ch, aliases) + fbonus
            if s>0: cands.append((s, str(fp), ch))
    if not cands: return []
    ...
    def rerank_key(item):
        s,src,ch=item
        t=ch.lower(); bonus=0
        if "consolidated statements of income" in t or "statements of operations" in t or "income statement" in t: bonus+=5
        if likely_table(ch): bonus+=3
        if re.search(r"\((in|In)\s+(millions|thousands)\)", ch): bonus+=2
        return s+bonus
...
def ollama_pull(model: str, timeout: float = 900.0) -> None:
    ...
    req=urllib.request.Request("http://127.0.0.1:11434/api/pull",data=data,headers={"Content-Type":"application/json"},method="POST")
    urllib.request.urlopen(req, timeout=timeout).read()

def ollama_generate(model: str, prompt: str, timeout: float = 240.0) -> str:
    payload=json.dumps({"model":model,"prompt":prompt,"stream":True,"options":{"num_ctx":32768}}).encode("utf-8")
    def _call():
    ...
    try: return _call()
        ...
        if e.code==404:
            ollama_pull(model); return _call()
        raise
...
def is_reasonable(inc: Dict[str,Any]) -> bool:
    ...
    if rev is None or rev <= 0: return False
    if cor is not None and (cor < 0 or cor > 2.5*rev): return False
    if opi is not None and abs(opi) > 2.0*rev: return False
    if ni  is not None and abs(ni)  > 2.0*rev: return False
    return True

def build_output(ok: bool, inc: Optional[Dict[str,Any]]=None, period: Optional[Dict[str,Any]]=None,
                 citations: Optional[List[Dict[str,str]]]=None, reason: Optional[str]=None) -> Dict[str,Any]:
    if not ok:
    ...
    def add(name,val,eps=False):
        v=coerce_float(val)
        if v is not None: metrics.append({"name":name,"value":v,"unit":"USD/share" if eps else "USD"})
...
def main():
    ...
    if not index_dir.exists():
        print(json.dumps({"answer":"Not enough evidence to answer.","metrics":[],"error":f"index-dir not found: {index_dir}"})); return
    ...
    for fp in files:
        raw=safe_read(fp)
        if raw and file_is_company(fp, raw, aliases, primary): company_files.append(fp)

    if args.debug:
        print(f"[DEBUG] Candidate files for {primary}: {len(company_files)}", file=sys.stderr)
        for fp in company_files: print(f"  - {fp}", file=sys.stderr)
    ...
    # Deterministic first (both orientations)
    if contexts:
        det = parse_income_windows_det(contexts, prefer="left")
        if det:
            scale = unit_scale("\n\n".join(contexts))
            inc = {k:(coerce_float(det.get(k))*scale if det.get(k) is not None else None)
            ...
            if not args.no_sanity and not is_reasonable(inc):
                if args.debug: print("[DEBUG] Sanity failed with first orientation; retrying with opposite orientation.", file=sys.stderr)
                det2 = parse_income_windows_det(contexts, prefer="right")
                if det2:
                    inc = {k:(coerce_float(det2.get(k))*scale if det2.get(k) is not None else None)
                           for k in ("netRevenue","costOfRevenue","operatingIncome","netIncome")}
                    inc["basicEPS"]=det2.get("basicEPS"); inc["dilutedEPS"]=det2.get("dilutedEPS")
                    if args.no_sanity or is_reasonable(inc):
                        print(json.dumps(build_output(True, inc, None, citations), indent=2)); return
            else:
                print(json.dumps(build_output(True, inc, None, citations), indent=2)); return
    ...
    if not contexts:
        ...
        if not chunks:
            print(json.dumps(build_output(False, reason="No candidate chunks matched income statement patterns or aliases"), indent=2)); return
    ...
    try:
        ...
        print(json.dumps(build_output(False, reason=f"Ollama error: {e}", citations=citations), indent=2)); return
    ...
    try:
        ...
        if args.debug: reason += " (see raw model response above)"
        print(json.dumps(build_output(False, reason=reason, citations=citations), indent=2)); return

    inc=(data or {}).get("incomeStatement", {}) or {}
    scale=unit_scale(context)
    for k in ("netRevenue","costOfRevenue","operatingIncome","netIncome"):
        if inc.get(k) is not None:
            v=coerce_float(inc[k]); inc[k]=None if v is None else v*scale

    if not args.no_sanity and not is_reasonable(inc):
        print(json.dumps(build_output(False, reason="Sanity check failed (LLM)", citations=citations), indent=2)); return
